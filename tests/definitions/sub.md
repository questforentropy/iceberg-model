...
| SUB-16 | U1 | the ledger IS the universal wave function stored factorized: random circuit universes run as global 2^n table vs contract DAG (merge = tensor, settlement = re-factorize), event-keyed draws → outcomes bit-identical, re-tensored DAG = Ψ entry-by-entry (~1e-15) |
